...
public class EmpRepository implements IEmpRepository {
	...
	public void updateEmp(EmpVO emp) {
		...
		//수정할 정보를 가져와서 업데이트하는 작업 ? 순서대로 작성 (get메서드)
		jdbcTemplate.update(sql, emp.getFirstName(), emp.getLastName(),emp.getEmail(),emp.getPhoneNumber(),emp.getHireDate(),emp.getJobId(),emp.getSalary(), 
	...
	public void insertEmp(EmpVO emp) {
		String sql = "insert into employees (employee_id, first_name, last_name, email, phone_number, hire_date, "
				...
				+ "values (?,?,?,?,?,sysdate,?,?,?,?,?)";
		jdbcTemplate.update(sql, emp.getEmployeeId(), emp.getFirstName(), emp.getLastName(),emp.getEmail(),emp.getPhoneNumber(),emp.getJobId(),emp.getSalary(), 
	...
	public List<Map<String, Object>> getAllManagerId() {
		String sql = "select d.manager_id, e.first_name from departments d join employees e on d.manager_id = employee_id order by d.manager_id";
		return jdbcTemplate.queryForList(sql);
	}
